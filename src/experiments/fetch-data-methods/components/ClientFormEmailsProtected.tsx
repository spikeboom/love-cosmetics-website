...
export default function ClientFormEmailsProtected() {
  const [state, setState] = useState<FormState | null>(null)
  const [isPending, startTransition] = useTransition()

  const handleSubmit = () => {
    startTransition(() => {
      getCustomerEmailsProtected()
  ...
  return (
    <div>
      ...
      >
        {isPending ? 'Carregando...' : 'Client → Server Action Protegida (Form)'}
      </button>
          ...
          {state.success ? (
            <>
              <div className="mb-4 space-y-2">
                <p className="text-green-600 font-semibold">✓ Client Form → Server Action Protegida</p>
                <p>Total de clientes: {state.count}</p>
                {state.authenticatedAs && (
                  <p className="text-blue-600">Autenticado como: {state.authenticatedAs}</p>
                )}
              </div>
